...
// - Vários títulos abertos: +15 se o cliente tem mais de um título em aberto
//   ao mesmo tempo (regra do documento original do projeto; não conta por título).
//
...
export const PONTOS_VARIOS_ABERTOS = 15;
...
export function calcularRisco(titulos, ultimoPagamento, config) {
  ...
  if (titulos.length === 0) {
    return {
      ...
      partes: { atraso: 0, perfil: 0, mudanca: 0, varios: 0 },
      maximos,
    };
  }
  ...
  const pontosVarios = abertos.length > 1 ? PONTOS_VARIOS_ABERTOS : 0;

  const nota = arredondar(
    Math.min(100, pontosAtraso + pontosPerfil + pontosMudanca + pontosVarios)
  );
  ...
  if (pontosVarios > 0) {
    motivos.push(`${abertos.length} títulos em aberto ao mesmo tempo`);
  }
  if (mudouPadrao) {
