import { calcularRisco } from './risco';
...
async function buscarTudo(criarConsulta) {
  const linhas = [];
...
// Devolve { riscoDe, erro }: riscoDe(clienteId) dá o risco do cliente, ou null
// se algo falhou (erro traz a mensagem). Uma falha aqui não deve derrubar a
// tela: quem chama mostra o aviso e segue.
  ...
  try {
    ...
    const titulos = await buscarTudo(() => {
      const consulta = supabase
        ...
        .select('id, cliente_id, data_vencimento, status, dias_atraso')
        .order('id');
    ...
    return { riscoDe: () => null, erro: e.message || 'erro desconhecido' };
  }
}
