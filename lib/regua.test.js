...
test('"antes do vencimento" pendente (ainda não mandado) já conta como enfileirado', () => {
  const envios = montarRegua({
    titulos: [titulo({ id: 't1', data_vencimento: '2026-09-24', status: 'em_aberto' })],
    clientes: [CLIENTE],
    mensagensEnviadas: [{ tipo: 'antes_vencimento', titulo_id: 't1', cliente_id: 'c1', enviado_em: null }], // pendente
    hoje: HOJE,
    diasAntesAviso: 3,
    diasRepetirCobranca: 7,
  });
...
test('"atraso" pendente (ainda não mandado) não gera outra cobrança pro mesmo cliente', () => {
  const envios = montarRegua({
    titulos: [titulo({ id: 't1', status: 'atrasado', dias_atraso: 5, data_vencimento: '2026-09-16' })],
    clientes: [CLIENTE],
    mensagensEnviadas: [{ tipo: 'atraso', titulo_id: 't1', cliente_id: 'c1', enviado_em: null }], // pendente
    hoje: HOJE,
    diasAntesAviso: 3,
    diasRepetirCobranca: 7,
  });
