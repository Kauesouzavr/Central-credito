...
export function montarHoje({ titulos, clientes, cobrancas, riscoDe, diasRepetirCobranca, hoje }) {
  ...
  let atrasoMaisAntigoCliente = null;
  ...
  for (const [clienteId, situacao] of situacoes) {
    ...
    if (situacao.atrasados > 0) {
      ...
      if (situacao.maiorAtraso > atrasoMaisAntigo) {
        atrasoMaisAntigo = situacao.maiorAtraso;
        atrasoMaisAntigoCliente = cliente.nome;
      }
  ...
  return {
    ...
    totalAberto: reais(totalAbertoCentavos),
    clientesAtencao: atencao.size,
    nuncaCobrados,
    atrasoMaisAntigo,
    atrasoMaisAntigoCliente,
